...
from gemstone import Core
from canal.graph import (SwitchBoxSide, SwitchBoxIO, InterconnectPolicy,
                         InterconnectGraph, DisjointSwitchBox, WiltonSwitchBox,
                         ImranSwitchBox, Tile, SwitchBox, CoreConnectionType)
from canal.generators import CoreInterface
...
def compute_num_tracks(x_offset: int, y_offset: int,
    ...
    """Computes the num of tracks needed for (x, y), given the track info."""
    x_diff = x - x_offset
    y_diff = y - y_offset
    result = 0
    for length, num_track in track_info.items():
        if x_diff % length == 0 and y_diff % length == 0:  # this tile
            result += num_track
    return result
...
def create_uniform_interconnect(width: int,
                                ...
                                port_connections: Dict[
                                    str,
                                    List[Tuple[SwitchBoxSide, SwitchBoxIO]]
                                ],
                                ...
                                additional_core_fn: Callable[[int, int],
                                                             Core]=_empty_fn,
    ...
    # Based on the IO sides specified. Inclusive once it's assigned to None,
    # nullify everything.
    ...
    for x in range(x_min, x_max + 1):
        for y in range(y_min, y_max + 1, tile_height):
            ...
            if additional_core is not None:
                ...
                tile_circuit.add_additional_core(
                    additional_core_interface,
                    CoreConnectionType.SB | CoreConnectionType.CB)

    # Create tiles without SB.
    for x in range(width):
        for y in range(height):
            # Skip if the tile is already created.
    ...
    # Set the actual interconnections, tracks sorted by length.
